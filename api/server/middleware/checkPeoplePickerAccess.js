...
const checkPeoplePickerAccess = async (req, res, next) => {
  try {
    ...
    const canViewUsers = allRoles.some(
      (role) => role?.permissions?.[PermissionTypes.PEOPLE_PICKER]?.[Permissions.VIEW_USERS] === true,
    );
    const canViewGroups = allRoles.some(
      (role) => role?.permissions?.[PermissionTypes.PEOPLE_PICKER]?.[Permissions.VIEW_GROUPS] === true,
    );
    const canViewRoles = allRoles.some(
      (role) => role?.permissions?.[PermissionTypes.PEOPLE_PICKER]?.[Permissions.VIEW_ROLES] === true,
    );
