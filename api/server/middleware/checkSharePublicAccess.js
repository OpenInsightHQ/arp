...
const resourceToPermissionType = {
  ...
  [ResourceType.GALLERY_ARTIFACT]: PermissionTypes.AGENTS,
};
...
const checkSharePublicAccess = async (req, res, next) => {
  try {
    ...
    const canSharePublic = allRoles.some(
      (role) => role?.permissions?.[permissionType]?.[Permissions.SHARE_PUBLIC] === true,
    );
