 ...
 * @param {boolean} [req.body.arg.echartsPrompt] - Optional echarts toggle state.
 * @param {string} [req.body.arg.updatedAt] - Updated timestamp for pinning/sorting.
 * @returns {object} 201 - The updated conversation object.
 */
router.post('/update', validateConvoAccess, async (req, res) => {
  const { conversationId, title, echartsPrompt, updatedAt } = req.body?.arg ?? {};
  ...
  const hasEchartsUpdate = echartsPrompt !== undefined;
  const hasUpdatedAtUpdate = updatedAt !== undefined;

  if (!hasTitleUpdate && !hasEchartsUpdate && !hasUpdatedAtUpdate) {
  ...
  if (hasEchartsUpdate && typeof echartsPrompt !== 'boolean') {
    return res.status(400).json({ error: 'echartsPrompt must be a boolean' });
  }

  if (hasUpdatedAtUpdate && typeof updatedAt !== 'string') {
    return res.status(400).json({ error: 'updatedAt must be a string' });
  }
  ...
  if (hasEchartsUpdate) {
    updatePayload.echartsPrompt = echartsPrompt;
  }
  if (hasUpdatedAtUpdate) {
    updatePayload.updatedAt = updatedAt;
  }

  try {
    const dbResponse = await saveConvo(req, updatePayload, {
