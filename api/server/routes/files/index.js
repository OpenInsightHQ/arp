...
const initialize = async () => {
  ...
  /**
   * Agent avatar uploads accept both JWT and Agent API key auth so external
   * admin systems can call the endpoint with a generated `sk-` key.
   * All other paths require JWT only.
   */
  router.use((req, res, next) => {
    if (req.path.startsWith('/images/agents/')) {
      return requireJwtOrApiKey(req, res, next);
    }
