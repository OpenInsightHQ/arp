...
router.delete('/', async (req, res) => {
  try {
    ...
    const { tool_resource, agent_id, assistant_id } = req.body;

    logger.debug(
      `[/files] DELETE request: files=${JSON.stringify(_files)}, tool_resource=${tool_resource}, agent_id=${agent_id}, assistant_id=${assistant_id}`,
    );
    ...
    logger.debug(
      `[/files] DB files: ${JSON.stringify(dbFiles.map((f) => ({ file_id: f.file_id, filename: f.filename, source: f.source, filepath: f.filepath, metadata: f.metadata })))}`,
    );

    /* Handle agent unlinking even if no valid files to delete.
     * Must run BEFORE the ownership-based early return below, otherwise
     * a missing DB file leaves a stale file_id in agent.tool_resources. */
...
router.get('/download/:userId/:file_id', fileAccess, async (req, res) => {
  try {
    ...
    const setHeaders = () => {
      ...
      const asciiFilename = cleanedFilename.replace(/[^\x20-\x7E]/g, '_');
      res.setHeader(
        'Content-Disposition',
...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    logger.info('[/files] Full metadata:', JSON.stringify(metadata));
    logger.info('[/files] req.body keys:', Object.keys(req.body));
    ...
    if (metadata.agent_id && metadata.tool_resource && !isMessageAttachment) {
      ...
      const fileRoles = req.user.roles ?? [req.user.role];
      if (!fileRoles.includes(SystemRoles.ADMIN)) {
