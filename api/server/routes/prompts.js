...
const patchPromptGroup = async (req, res) => {
  try {
    ...
    const promptRoles = req.user.roles ?? [req.user.role];
    if (promptRoles.includes(SystemRoles.ADMIN)) {
...
router.get('/', async (req, res) => {
  try {
    ...
    const query = { author };
    const promptListRoles = req.user.roles ?? [req.user.role];
    if (promptListRoles.includes(SystemRoles.ADMIN)) {
      delete query.author;
    }
