...
router.get('/:roleName', async (req, res) => {
  ...
  const reqRoles = req.user.roles ?? [req.user.role];
  const isAdmin = reqRoles.includes(SystemRoles.ADMIN);
  if (
    (!isAdmin && roleName === SystemRoles.ADMIN) ||
    (!isAdmin && !roleDefaults[roleName])
  ) {
