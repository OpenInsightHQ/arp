...
const { getStreamLogCollector, removeStreamLogCollector } = require('~/server/services/StreamLog');
...
router.get('/chat/status/:conversationId', async (req, res) => {
  ...
  res.json({
    active: isActive,
    streamId: sanitizeReflectedString(conversationId),
    status: job.status,
...
router.post('/chat/abort', async (req, res) => {
  ...
  if (job && jobStreamId) {
    ...
    if (
      ...
      const streamLogCollector = getStreamLogCollector(jobStreamId);
      ...
      removeStreamLogCollector(jobStreamId);
    }
