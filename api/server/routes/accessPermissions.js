...
/**
 * Resolve galleryArtifactId (e.g. gal_xxx) or MongoDB ObjectId to MongoDB _id
 */
const findGalleryArtifactByObjectId = async (galleryArtifactId) => {
  const mongoose = require('mongoose');
  
  // 如果传入的是 ObjectId 格式（24 位 hex 字符串），直接查询 _id
  if (mongoose.Types.ObjectId.isValid(galleryArtifactId)) {
    const artifact = await mongoose.model('GalleryArtifact')
      .findOne({ _id: galleryArtifactId })
      .lean();
    return artifact ? { _id: artifact._id } : null;
  }
  
  // 否则查询 galleryArtifactId 字段（如 gal_xxx）
  const artifact = await mongoose.model('GalleryArtifact')
    .findOne({ galleryArtifactId })
    .lean();
  return artifact ? { _id: artifact._id } : null;
};
...
const checkResourcePermissionAccess = (requiredPermission) => (req, res, next) => {
  ...
  } else if (resourceType === ResourceType.GALLERY_ARTIFACT) {
    middleware = canAccessResource({
      resourceType: ResourceType.GALLERY_ARTIFACT,
      requiredPermission,
      resourceIdParam: 'resourceId',
      idResolver: findGalleryArtifactByObjectId,
    });
  } else {
