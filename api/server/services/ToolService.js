...
async function loadToolDefinitionsWrapper({
  ...
  const isPIAvailable = isPIConfigured(req);
  const hasExecuteCodeCap = checkCapability(AgentCapabilities.execute_code);
  ...
  // Ensure gallery tools are always included for solidification

  for (const galleryTool of galleryTools) {
    if (!filteredTools.includes(galleryTool)) {
      filteredTools.push(galleryTool);
    }
  }
  ...
  if (hasWebSearch) {
    const dbWebSearchPrompt = await getSystemPromptOrSeed('tool_context.web_search');
    toolContextMap[Tools.web_search] = dbWebSearchPrompt ?? buildWebSearchContext();
  }
...
async function loadAgentTools({
  ...
  logger.info(
    `[loadAgentTools] definitionsOnly: ${definitionsOnly}, conversationId: ${conversationId || 'not provided'}`,
  );
  ...
  for (const galleryTool of galleryTools) {
    if (!_agentTools.includes(galleryTool)) {
      _agentTools.push(galleryTool);
    }
  }
...
async function loadToolsForExecution({
  ...
  logger.debug(
    `[loadToolsForExecution] isToolSearch: ${isToolSearch}, toolRegistry: ${toolRegistry?.size ?? 'undefined'}, conversationId: ${conversationId || 'not provided'}`,
  );
