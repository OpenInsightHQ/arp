...
async function createMCPTools({
  ...
  if (serverConfig?.url) {
    ...
    const isDomainAllowed =
      Array.isArray(allowedDomains) && allowedDomains.includes('*')
        ? true
        : await isMCPDomainAllowed(serverConfig, allowedDomains);
    if (!isDomainAllowed) {
 ...
 * @param {string} [role] - The user's role (ADMIN bypasses ACL)
 * @returns {Object} Object containing mcpConfig, appConnections, userConnections, and oauthServers
 */
async function getMCPSetupData(userId, role) {
  const mcpConfig = await getMCPServersRegistry().getAllServerConfigs(userId, role);
  ...
  const oauthServers = await getMCPServersRegistry().getOAuthServers(userId, role);
