...
const { deletePIFile, isPIConfigured, PI_HOST } = require('~/server/services/PIService');
  ...
  for (const file of files) {
    ...
    if (source === FileSources.execute_code) {
      const fileIdentifier =
        file.metadata?.fileIdentifier ?? file.filepath?.split('/api/files/code/download/')[1] ?? '';
      const [pathPart, queryString] = fileIdentifier.split('?');
      const [sessionId, ...rest] = pathPart.split('/');
      const filename = file.filename ?? rest.join('/');

      // Parse entity_id from query string (e.g., "entity_id=GLM-qingsan__GLM-4.7___GLM")
      let agentId = req.body.agent_id || 'default';
      if (queryString) {
        const params = new URLSearchParams(queryString);
        const parsedAgentId = params.get('entity_id');
        if (parsedAgentId) {
          agentId = parsedAgentId;
        }
      }

      if (sessionId && isPIConfigured(req)) {
        promises.push(
          (async () => {
            try {
              await deletePIFile({ sessionId, filename, agentId }, req.user.id);
            } catch (err) {
              logger.error(`[processDeleteRequest] Error deleting PI file: ${err.message}`);
            }
          })(),
        );
      }
...
const processAgentFileUpload = async ({ req, res, metadata }) => {
  ...
  logger.debug(
    `[processAgentFileUpload] metadata: agent_id=${agent_id}, tool_resource=${tool_resource}, conversationId=${conversationId}, model=${model}, ephemeralAgentId=${ephemeralAgentId}, endpoint=${endpoint}, modelLabel=${modelLabel}`,
  );

  logger.info(
    `[processAgentFileUpload] File info: originalname=${file.originalname}, mimetype=${file.mimetype}, size=${file.size}`,
  );

  let messageAttachment = !!metadata.message_file;

  if (!file.mimetype) {
    throw new Error(`File mimetype is missing for ${file.originalname}`);
  }
  ...
  /**
   * Message attachments with text-parsable MIME types (text/plain, text/html,
   * application/json, etc.) go through the same text-extraction path as the
   * explicit `context` tool resource so the content actually reaches the LLM
   * via `extractFileContext`. OCR/STT/document types are excluded so PDFs,
   * audio, and other binary attachments keep their existing message-send flow.
   */
  const isMessageTextAttachment =
    messageAttachment &&
    !fileConfig.checkType(file.mimetype, fileConfig.ocr?.supportedMimeTypes || []) &&
    !fileConfig.checkType(file.mimetype, fileConfig.stt?.supportedMimeTypes || []) &&
  ...
  if (tool_resource === EToolResources.execute_code) {
    ...
    if (!conversationId) {
      throw new Error(
        'Please start a conversation first before uploading files for code execution.',
      );
    }

    const uploadEntityId = entity_id || '';

    logger.info(
      `[processAgentFileUpload] Uploading execute_code file: conversationId=${conversationId}, entityId=${uploadEntityId}`,
    );
  ...
  } else if (tool_resource === EToolResources.context || isMessageTextAttachment) {
    const { file_id, temp_file_id = null } = metadata;
  ...
  let source = getFileStrategy(appConfig, { isImage: isImageFile });

  // Use execute_code source for code interpreter files
  if (tool_resource === EToolResources.execute_code) {
  ...
  if (tool_resource === EToolResources.file_search) {
    ...
    fileInfoMetadata = {};
  } else if (tool_resource === EToolResources.execute_code) {
    // execute_code files already uploaded to PI via uploadCodeEnvFile above
    // Use placeholder values - file already uploaded, no local storage needed
    storageResult = {
      bytes: 0,
      filename: file.originalname,
      filepath: `/api/files/code/download/${fileInfoMetadata?.fileIdentifier || ''}`,
      embedded: false,
    };
  } else {
...
function filterFile({ req, image, isAvatar }) {
  const { file } = req;
  let { endpoint, endpointType, file_id, width, height } = req.body;
