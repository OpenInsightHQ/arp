...
const { EToolResources, FileSources, FileContext, AgentCapabilities } = require('librechat-data-provider');
...
describe('processAgentFileUpload', () => {
  ...
  describe('message attachment text extraction', () => {
    ...
    test('extracts text for text/plain message attachment and stores as FileSources.text', async () => {
      const parsedText = 'hello world';
      const parsedBytes = Buffer.byteLength(parsedText, 'utf8');
      const { parseText } = require('@librechat/api');
      parseText.mockResolvedValueOnce({ text: parsedText, bytes: parsedBytes });

      mergeFileConfig.mockReturnValue(
        makeFileConfig({ textSupportedMimeTypes: ['text/plain'] }),
      );
      const req = makeReq({ mimetype: 'text/plain', ocrConfig: null });
      ...
      expect(parseText).toHaveBeenCalledWith(
        expect.objectContaining({ file: req.file, file_id: 'file-uuid-123' }),
      );
      ...
      const { addAgentResourceFile } = require('~/models/Agent');
      expect(addAgentResourceFile).not.toHaveBeenCalled();
    });

    test('falls back to standard storage when text type is not supported', async () => {
      const { parseText } = require('@librechat/api');
      mergeFileConfig.mockReturnValue(makeFileConfig({ textSupportedMimeTypes: [] }));
      const req = makeReq({ mimetype: 'text/plain', ocrConfig: null });

      const storageUpload = jest
        .fn()
        .mockResolvedValue({ bytes: 12, filename: 'upload.bin', filepath: '/path/file' });
    ...
    test('does not extract text for OCR-supported message attachments (e.g. PDF)', async () => {
      const { parseText } = require('@librechat/api');
      mergeFileConfig.mockReturnValue(
        makeFileConfig({
          ocrSupportedMimeTypes: [PDF_MIME],
          textSupportedMimeTypes: [PDF_MIME],
        }),
      );
      const req = makeReq({ mimetype: PDF_MIME, ocrConfig: null });
    ...
    test('does not extract text when message_file flag is absent', async () => {
      const { parseText } = require('@librechat/api');
      mergeFileConfig.mockReturnValue(
        makeFileConfig({ textSupportedMimeTypes: ['text/plain'] }),
      );
