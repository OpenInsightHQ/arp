...
const { isStreamLogEnabled, createStreamLogCollector } = require('~/server/services/StreamLog');
 ...
 */
function createToolLoader(signal, streamId = null, conversationId = null, definitionsOnly = false) {
  /**
   * @param {object} params
...
const initializeClient = async ({ req, res, signal, endpointOption }) => {
  ...
  /** Raw stream log collector (null when `LOG_LLM_STREAM` is disabled). */
  const streamLogCollector = isStreamLogEnabled()
    ? createStreamLogCollector(streamId ?? undefined)
    : null;
  ...
  /** @type {{ value: number }} Tracks the highest LangGraph super-step reached during the run. */
  ...
  logger.info(
    `[initializeClient] conversationId: ${conversationId || 'not provided'}, from body: ${!!req.body.conversationId}`,
  );
