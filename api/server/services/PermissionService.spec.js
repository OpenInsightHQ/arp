...
describe('PermissionService', () => {
  ...
  // ROLE principal IDs are now roles._id (ObjectId), not role-name strings.
  // Tests below grant and read back ACLs by the same ObjectId for consistency.
  const adminRoleId = new mongoose.Types.ObjectId();
  const editorRoleId = new mongoose.Types.ObjectId();
  const userRoleId = new mongoose.Types.ObjectId();
  const moderatorRoleId = new mongoose.Types.ObjectId();
  ...
  describe('bulkUpdateResourcePermissions', () => {
    ...
    test('should grant permission to a role', async () => {
      ...
      expect(entry.principalId.toString()).toBe(adminRoleId.toString());
    ...
    test('should check permissions for user with role', async () => {
      ...
      getUserPrincipals.mockResolvedValue([
        { principalType: PrincipalType.USER, principalId: userId },
        { principalType: PrincipalType.ROLE, principalId: adminRoleId },
        { principalType: PrincipalType.PUBLIC },
      ]);
    ...
    test('should optimize permission checks when role is provided', async () => {
      ...
      getUserPrincipals.mockResolvedValue([
        ...
        { principalType: PrincipalType.ROLE, principalId: editorRoleId },
        { principalType: PrincipalType.PUBLIC },
      ]);
    ...
    test('should handle role changes dynamically', async () => {
      ...
      getUserPrincipals.mockResolvedValue([
        { principalType: PrincipalType.USER, principalId: testUserId },
        { principalType: PrincipalType.ROLE, principalId: userRoleId },
        { principalType: PrincipalType.PUBLIC },
      ]);
  ...
  describe('String vs ObjectId Edge Cases', () => {
    ...
    test('should handle ObjectId roleId in grantPermission for ROLE type', async () => {
      const entry = await grantPermission({
        principalType: PrincipalType.ROLE,
        principalId: moderatorRoleId,
        resourceType: ResourceType.AGENT,
        resourceId: testResourceId,
        accessRoleId: AccessRoleIds.AGENT_VIEWER,
        grantedBy: grantedById,
      ...
      expect(entry).toBeDefined();
      expect(entry.principalType).toBe(PrincipalType.ROLE);
      // ROLE principalId is stored as ObjectId (roles._id)
      expect(entry.principalId).toBeInstanceOf(mongoose.Types.ObjectId);
      expect(entry.principalId.toString()).toBe(moderatorRoleId.toString());
      expect(entry.principalModel).toBe(PrincipalModel.ROLE);
    });
    ...
    test('should handle bulkUpdateResourcePermissions with string IDs', async () => {
      const updatedPrincipals = [
        ...
        },
        {
          ...
          id: adminRoleId.toString(), // ObjectId hex string (roles._id)
          accessRoleId: AccessRoleIds.AGENT_OWNER,
        },
      ];
      ...
      // Verify ROLE entry has ObjectId
      const roleEntry = await AclEntry.findOne({
        principalType: PrincipalType.ROLE,
        resourceType: ResourceType.AGENT,
        resourceId: testResourceId,
      });
      expect(roleEntry.principalId).toBeInstanceOf(mongoose.Types.ObjectId);
      expect(roleEntry.principalId.toString()).toBe(adminRoleId.toString());
    });
