...
async function createActionTool({
  ...
  const _call = async (toolInput, config) => {
    try {
      ...
      const contextUserId =
        config?.configurable?.user?.id || config?.configurable?.user_id || userId;
      const contextAgentId = config?.configurable?.agent_id;
      const contextConversationId =
        config?.configurable?.thread_id || config?.configurable?.requestBody?.conversationId;

      const contextHeaders = {};
      if (contextUserId) {
        contextHeaders['X-User-Id'] = contextUserId;
      }
      if (contextAgentId) {
        contextHeaders['X-Agent-Id'] = contextAgentId;
      }
