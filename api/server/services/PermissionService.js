...
const grantPermission = async ({
  ...
  try {
    ...
    // Validate principalId — all non-PUBLIC principals require a valid ObjectId
    if (
      principalType &&
  ...
  try {
    ...
    for (const principal of updatedPrincipals) {
      try {
        ...
        if (principal.type !== PrincipalType.PUBLIC) {
          query.principalId = new mongoose.Types.ObjectId(principal.id);
        }
        ...
        const update = {
          ...
          $setOnInsert: {
            ...
            ...(principal.type !== PrincipalType.PUBLIC && {
              principalId: new mongoose.Types.ObjectId(principal.id),
