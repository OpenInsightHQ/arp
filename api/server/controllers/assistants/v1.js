...
const patchAssistant = async (req, res) => {
  try {
    ...
    if (append_current_datetime !== undefined) {
      const datetimeFlag = Boolean(append_current_datetime);
      await updateAssistantDoc({ assistant_id }, { append_current_datetime: datetimeFlag });
      updatedAssistant.append_current_datetime = datetimeFlag;
    }
