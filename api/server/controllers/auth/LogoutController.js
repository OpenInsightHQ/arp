    ...
    if (
      ...
      if (!openIdConfig) {
        ...
        if (endSessionEndpoint) {
          ...
          const defaultRedirectUri = `${process.env.DOMAIN_CLIENT}/login`;
          const rawRedirectUri = process.env.OPENID_POST_LOGOUT_REDIRECT_URI;
          let postLogoutRedirectUri = defaultRedirectUri;
          if (typeof rawRedirectUri === 'string' && rawRedirectUri.trim() !== '') {
            try {
              postLogoutRedirectUri = new URL(rawRedirectUri.trim()).toString();
            } catch {
              logger.warn(
                '[logoutController] Invalid OPENID_POST_LOGOUT_REDIRECT_URI, using default.',
              );
            }
          }
