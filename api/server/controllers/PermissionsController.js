...
const updateResourcePermissions = async (req, res) => {
  try {
    ...
    logger.info(`[updateResourcePermissions] resourceType: ${resourceType}, resourceId: ${resourceId}`);
    logger.info(`[updateResourcePermissions] req.body: ${JSON.stringify(req.body)}`);
...
const getResourcePermissions = async (req, res) => {
  try {
    ...
    const results = await AclEntry.aggregate([
      ...
      // Lookup Role information (for role principals — principalId is roles._id)
      {
        $lookup: {
          from: 'roles',
          localField: 'principalId',
    ...
    for (const result of results) {
      ...
      } else if (result.principalType === PrincipalType.ROLE && result.roleInfo) {
        principals.push({
          type: PrincipalType.ROLE,
          /** Role ObjectId as ID (matches searchPrincipals / getUserPrincipals) */
          id: result.roleInfo._id.toString(),
          /** Display the role name */
          name: result.roleInfo.name,
          description: `System role: ${result.roleInfo.name}`,
          source: 'local',
          idOnTheSource: result.roleInfo.name,
          accessRoleId: result.accessRoleId,
        });
      }
    }
