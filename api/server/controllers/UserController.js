...
const { deleteUserAgents, getAgents } = require('~/models/Agent');
...
const getUserController = async (req, res) => {
  ...
  /** @type {IUser} */
  const rawUser = req.user.toObject != null ? req.user.toObject() : { ...req.user };
  /**
   * Strip sensitive fields via destructuring so they are never included in the
   * response payload. refreshToken is not `select: false` in the schema, so it
  ...
  if (userData.favorites && Array.isArray(userData.favorites)) {
    const agentFavorites = userData.favorites.filter((fav) => fav.agentId);
    if (agentFavorites.length > 0) {
      const agentIds = agentFavorites.map((fav) => fav.agentId);
      const agents = await getAgents({ id: { $in: agentIds } });
      const agentMap = new Map(agents.map((agent) => [agent.id, agent]));
      const validAgentIds = agents.map((agent) => agent._id);
      const permissionsMap = await getResourcePermissionsMap({
        userId: req.user.id,
        role: req.user.role,
        resourceType: ResourceType.AGENT,
        resourceIds: validAgentIds,
      });
      const validFavorites = [];
      for (const fav of agentFavorites) {
        const agent = agentMap.get(fav.agentId);
        if (!agent) {
          continue;
        }
        const permission = permissionsMap.get(agent._id.toString()) ?? 0;
        if ((permission & 1) === 0) {
          continue;
        }
        validFavorites.push(fav);
      }
      const otherFavorites = userData.favorites.filter((fav) => !fav.agentId);
      const updatedFavorites = [...validFavorites, ...otherFavorites];
      if (updatedFavorites.length !== userData.favorites.length) {
        userData.favorites = updatedFavorites;
        await updateUser(userData.id, { favorites: updatedFavorites });
      }
    }
  }
