...
const { isStreamLogEnabled } = require('~/server/services/StreamLog');
...
class BaseClient {
  ...
  async sendMessage(message, opts = {}) {
    ...
    const responseMessage = {
      ...
      ...(this.lastRecursionLimit != null && {
        recursionLimit: `${this.stepRef?.value ?? 0}/${this.lastRecursionLimit}`,
      }),
      ...(this.metadata ?? {}),
    ...
    if (
      ...
      let usage = this.getStreamUsage != null ? this.getStreamUsage() : null;
      // Fallback: global store set by OpenAI streaming handler
      if (!usage && global.__lcUsage && global.__lcUsage.length > 0) {
        usage = global.__lcUsage.pop();
      }
    ...
    // Keep tokenCount and inputTokenCount for SSE final event
  ...
  async saveMessageToDatabase(message, endpointOptions, user = null) {
    ...
    const fieldsToKeep = {
      ...
      ...(message.finish_reason !== undefined && { finish_reason: message.finish_reason }),
      ...endpointOptions,
    };
