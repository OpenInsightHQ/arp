...
module.exports = {
  ...
  getConvosByCursor: async (
    ...
    if (search) {
      ...
      if (searchEnabled) {
        try {
          const convoResults = await Conversation.meiliSearch(search, {
            filter: `user = "${user}"`,
          });
          const convoIds = Array.isArray(convoResults.hits)
            ? convoResults.hits.map((result) => result.conversationId)
            : [];
          convoIds.forEach((id) => allMatchingIds.add(id));
        } catch (error) {
          logger.warn(
            '[getConvosByCursor] Error searching conversations via MeiliSearch:',
            error.message,
          );
        }
        ...
        if (searchMessages) {
          try {
            ...
            const messagesIndex = meiliClient.index('messages');
            const messageResults = await messagesIndex.search(search, {
              filter: `user = "${user}"`,
              limit: 1000,
            });
            const messageIds = Array.isArray(messageResults.hits)
              ? messageResults.hits.map((result) => result.conversationId)
              : [];
            messageIds.forEach((id) => allMatchingIds.add(id));
          } catch (error) {
            logger.warn(
              '[getConvosByCursor] Error searching messages via MeiliSearch:',
              error.message,
            );
          }
        }
        ...
        if (searchMessages) {
          try {
            ...
            const messageResults = await Message.find(
              ...
              .limit(1000)
              .lean();
            const messageIds = Array.isArray(messageResults)
              ? messageResults.map((result) => result.conversationId)
              : [];
            messageIds.forEach((id) => allMatchingIds.add(id));
          } catch (error) {
            logger.warn('[getConvosByCursor] Error searching messages via MongoDB:', error.message);
          }
        }
      }
      ...
      if (allMatchingIds.size === 0) {
        ...
        filters.push({ conversationId: { $in: Array.from(allMatchingIds) } });
      }
    }
    ...
    try {
      ...
      const convos = await Conversation.find(query)
        .select(
          'conversationId endpoint title createdAt updatedAt user model agent_id assistant_id spec iconURL messages',
        )
        .sort(sortObj)
        .limit(limit + 1)
        .lean();
