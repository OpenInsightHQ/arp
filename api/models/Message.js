...
async function saveMessage(req, params, metadata) {
  ...
  try {
    ...
    // Merge attachments: preserve non-pending memory suggestion statuses from DB
    // so that user confirm/dismiss actions are not overwritten during SSE final save
    if (update.attachments && Array.isArray(update.attachments)) {
      try {
        const existing = await Message.findOne({ messageId: params.messageId, user: req.user.id }).lean();
        if (existing && existing.attachments && Array.isArray(existing.attachments)) {
          update.attachments = update.attachments.map((att) => {
            const mem = att?.memory;
            if (!mem || mem.type !== 'suggestion' || mem.status === 'pending') {
              return att;
            }
            // This attachment has a non-pending status from SSE — check DB for user action
            const dbMatch = existing.attachments.find((dbAtt) => {
              const dbMem = dbAtt?.memory;
              return dbMem?.type === 'suggestion' && dbMem?.key === mem.key && dbMem.status !== 'pending';
            });
            // If DB has a user-confirmed status, keep it
            if (dbMatch) {
              return { ...att, memory: { ...mem, status: dbMatch.memory.status } };
            }
            return att;
          });
        }
      } catch (mergeErr) {
        logger.warn('Error merging attachments status, using update as-is:', mergeErr);
      }
    }
