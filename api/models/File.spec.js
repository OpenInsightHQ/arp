...
describe('File Access Control', () => {
  ...
  describe('Role-based file permissions', () => {
    it('should optimize permission checks when role is provided', async () => {
      ...
      // Create ADMIN role document (getUserPrincipals resolves ROLE principal to roles._id)
      const Role = mongoose.models.Role;
      const adminRoleDoc = await Role.create({ name: 'ADMIN', description: 'Administrator role' });

      // Grant permission to ADMIN role (by roles._id ObjectId)
      await grantPermission({
        principalType: PrincipalType.ROLE,
        principalId: adminRoleDoc._id,
        resourceType: ResourceType.AGENT,
        resourceId: agent._id,
        accessRoleId: AccessRoleIds.AGENT_EDITOR,
        grantedBy: authorId,
    ...
    it('should deny access when user role changes', async () => {
      ...
      // Create EDITOR role document (getUserPrincipals resolves ROLE principal to roles._id)
      const Role = mongoose.models.Role;
      const editorRoleDoc = await Role.create({ name: 'EDITOR', description: 'Editor role' });

      // Grant permission to EDITOR role only (by roles._id ObjectId)
      await grantPermission({
        principalType: PrincipalType.ROLE,
        principalId: editorRoleDoc._id,
        resourceType: ResourceType.AGENT,
        resourceId: agent._id,
        accessRoleId: AccessRoleIds.AGENT_EDITOR,
        grantedBy: authorId,
