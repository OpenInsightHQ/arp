...
import type { IGroup, IRole, IUser, IUserRole } from '~/types';
...
export function createUserGroupMethods(mongoose: typeof import('mongoose')) {
   ...
   *
   * Resolves ALL roles for the user — primary `user.role` plus any additional
   * roles from the `userroles` collection — and maps each to its `roles._id`
   * (ObjectId) so ROLE principals are consistent with USER/GROUP principals.
   *
   * When department-based authorization is added, this function will also
   * collect department-assigned roles as additional principals.
   *
  ...
  async function getUserPrincipals(
    ...
    // Collect all role names: primary role + additional roles from userroles collection
    ...
    if (UserRole) {
      const urQuery = UserRole.findOne({ userId: userObjectId }).select('roleNames');
      if (session) {
        urQuery.session(session);
      }
      const userRoleDoc = await urQuery.lean();
      userRoleDoc?.roleNames?.forEach((roleName) => {
    ...
    // Resolve role names → `roles._id` ObjectIds so ROLE principals are
    // consistent with USER/GROUP principals (which also use ObjectId). This
    // also makes ACLs survive role renames — a critical robustness property.
    if (roleNames.size > 0) {
      const Role = mongoose.models.Role as Model<IRole>;
      const roleQuery = Role.find({ name: { $in: [...roleNames] } }).select('_id');
      if (session) {
        roleQuery.session(session);
      }
      const roleDocs = await roleQuery.lean();
      roleDocs.forEach((roleDoc) => {
        principals.push({
          principalType: PrincipalType.ROLE,
          principalId: roleDoc._id as Types.ObjectId,
        });
      });
    }
  ...
  async function searchPrincipals(
    ...
    if (!typeFilter || typeFilter.includes(PrincipalType.ROLE)) {
      ...
      if (Role) {
        const regex = new RegExp(trimmedPattern, 'i');
        const roleQuery = Role.find({ name: regex }).select('_id name').limit(limitPerType);
