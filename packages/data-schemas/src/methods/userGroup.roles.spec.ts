...
describe('Role-based Permissions Integration', () => {
  describe('getUserPrincipals with roles', () => {
    test('should include role principal for user with role', async () => {
      ...
      // ROLE principal now resolves to roles._id (ObjectId), not the role name string
      expect(rolePrincipal?.principalId).toBeInstanceOf(mongoose.Types.ObjectId);
      expect(rolePrincipal?.principalId?.toString()).toBe(
    ...
    test('should not include role principal when role name has no matching Role document', async () => {
      // user.role references a name that does not exist in the roles collection;
      // since ROLE principals are resolved to roles._id, an unmatched name yields
      // no ROLE principal.
      const user = await User.create({
        name: 'Phantom Role User',
      ...
      expect(principals).toHaveLength(2);
      expect(principals.find((p) => p.principalType === PrincipalType.ROLE)).toBeUndefined();
    });
    ...
    test('should include all principal types for user with role and groups', async () => {
      ...
      // Check role principal — resolved to roles._id ObjectId
      const rolePrincipal = principals.find((p) => p.principalType === PrincipalType.ROLE);
      expect(rolePrincipal?.principalId).toBeInstanceOf(mongoose.Types.ObjectId);
      expect(rolePrincipal?.principalId?.toString()).toBe(
        (moderatorRole._id as mongoose.Types.ObjectId).toString(),
      );
    ...
    test('should handle different role values', async () => {
      ...
      // Create a Role document for each name so the resolver can find an _id
      const roleById: Record<string, mongoose.Types.ObjectId> = {};
      for (const testCase of testCases) {
        const created = await Role.create({ name: testCase.role, description: `${testCase.role} role` });
        roleById[testCase.role] = created._id as mongoose.Types.ObjectId;
      }
      ...
      for (const testCase of testCases) {
        ...
        expect(rolePrincipal).toBeDefined();
        // Resolves to roles._id (ObjectId), not the name
        expect(rolePrincipal?.principalId).toBeInstanceOf(mongoose.Types.ObjectId);
        expect(rolePrincipal?.principalId?.toString()).toBe(roleById[testCase.role].toString());
      }
    });
  });
  ...
  describe('searchPrincipals with role support', () => {
    ...
    test('should search for roles when Role model exists', async () => {
      ...
      // searchPrincipals returns roles._id (ObjectId hex) as id, name as name
      expect(roleResults[0].id).toMatch(/^[a-f\d]{24}$/i);
