...
describe('AclEntry Model Tests', () => {
  ...
  describe('String vs ObjectId Edge Cases', () => {
    ...
    test('should handle ObjectId roleId in grantPermission for ROLE type', async () => {
      // ROLE principalId is now roles._id (ObjectId), consistent with USER/GROUP
      const roleObjectId = new mongoose.Types.ObjectId();
      ...
      expect(entry?.principalId).toBeInstanceOf(mongoose.Types.ObjectId);
      expect(entry?.principalId?.toString()).toBe(roleObjectId.toString());
