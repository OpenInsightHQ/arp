...
Use this tool ONLY for files listed with kind="text" in the <attachments> section of the system prompt (also referenced by the user as [附件:filename] or [Attachment:filename]).

Rules:
- path MUST be a kind="text" <path> value from <attachments>, passed EXACTLY as listed (workspace-relative, e.g. report.txt, data/values.csv). Do not invent paths.
...
export function getToolDefinition(toolName: string): ToolRegistryDefinition | undefined {
  return toolDefinitions[toolName] ?? agentToolDefinitions[toolName] ?? piToolDefinitions[toolName];
}
...
export function getAllToolDefinitions(): ToolRegistryDefinition[] {
  return [
    ...
    ...Object.values(piToolDefinitions),
  ];
}
