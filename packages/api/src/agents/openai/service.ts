...
export async function createAgentChatCompletion(
  ...
  try {
    ...
    // Check if content moderation error — show user-friendly message instead of raw error
    const isModerationError =
      ...
      errorMessage.includes('flagged by our moderation');

    const userFriendlyMessage = isModerationError
      ? '{"type":"moderation"}'
      : `Error: ${errorMessage}`;
    ...
    if (res.headersSent) {
      ...
      if (isModerationError) {
        // Send structured moderation error so frontend Error.tsx can render localized message
