...
export class ServerConfigsDB implements IServerConfigsRepositoryInterface {
   ...
   * @param role - the user's role. ADMIN users bypass ACL checks.
   ...
   * @param role optional user role. ADMIN users bypass ACL and get all servers without consumeOnly.
  ...
  public async getAll(
    ...
    if (role === SystemRoles.ADMIN) {
      logger.debug(
        `[ServerConfigsDB.getAll] ADMIN bypass: returning all DB MCP servers for user ${userId}`,
      );
      const MCPServer = this._mongoose.model('MCPServer');
      const allData = (await MCPServer.find().lean()) as unknown as MCPServerDocument[];
      const parsedConfigs: Record<string, ParsedServerConfig> = {};
      const parsed = await Promise.all(
        allData.map((s) => this.mapDBServerToParsedConfig(s)),
      );
      allData.forEach((s, i) => {
        parsedConfigs[s.serverName] = parsed[i];
      });
