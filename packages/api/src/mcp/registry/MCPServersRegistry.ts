...
export class MCPServersRegistry {
  ...
  public async getServerConfig(
    ...
    const configFromDB = await this.dbConfigsRepo.get(serverName, userId, role);
  ...
  public async getAllServerConfigs(
    ...
    const fetchPromise = this.fetchAllServerConfigs(cacheKey, userId, role);
  ...
  public async updateServer(
    ...
    if (config.apiKey?.source === 'admin' && !config.apiKey?.key) {
      const existingConfig = await configRepo.get(serverName, userId, role);
  ...
  public async getOAuthServers(userId?: string, role?: string): Promise<Set<string>> {
    const allServers = await this.getAllServerConfigs(userId, role);
