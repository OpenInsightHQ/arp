...
export interface JwtOrApiKeyDependencies extends ApiKeyAuthDependencies {
  /** Existing JWT middleware (e.g. passport-based requireJwtAuth). */
  jwtAuth: RequestHandler;
}

/**
 * Combined auth middleware: routes the request to JWT or API-key validation
 * based on the `Authorization` header.
 *
 * - `Authorization: Bearer sk-...` → API key path (validates via `validateAgentApiKey`)
 * - Anything else (JWT, cookie, etc.) → delegates to the provided `jwtAuth` middleware
 *
 * Both paths populate `req.user` (and `req.apiKeyId` for the API-key path),
 * so downstream handlers work uniformly.
 */
export function createRequireJwtOrApiKeyAuth(deps: JwtOrApiKeyDependencies) {
  const requireApiKey = createRequireApiKeyAuth(deps);
  return (req: ApiKeyAuthRequest, res: Response, next: NextFunction) => {
    const authHeader = req.headers.authorization || '';
    if (authHeader.startsWith('Bearer sk-')) {
      return requireApiKey(req, res, next);
    }
    return deps.jwtAuth(req, res, next);
  };
}
