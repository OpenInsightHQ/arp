...
import { EModelEndpoint, eModelEndpointSchema, coerceNumber } from './schemas';
...
export const endpointSchema = baseEndpointSchema.merge(
  z.object({
    ...
    piMaxContextTokens: z.union([z.number().int().positive(), z.string()]).optional(),
  }),
);
...
export type TStartupConfig = {
  ...
  leftSidebarButtonHidden?: boolean;
  previewCodeHidden?: boolean;
...
export type TWatermarkConfig = {
  ...
  artifacts: boolean;
  template?: string;
  /** Text opacity in the 0-1 range. Defaults to 0.08 when unset. */
  opacity?: number;
  /** Font size in pixels. Defaults to 14 when unset. */
  fontSize?: number;
  /** Tiling density in the 1-10 range. Defaults to 5 (≈ 4×6 grid) when unset. */
  density?: number;
  /** Tilt angle in degrees, clamped to [-90, 90]. Defaults to -10 when unset. */
  rotation?: number;
};
