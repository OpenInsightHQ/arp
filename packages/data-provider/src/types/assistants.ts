...
export type AgentModelParameters = {
  ...
  toolCallVisible: AgentParameterValue;
...
export type PartMetadata = {
  ...
  groupId?: number;
  /** Timestamp (ms) when this content part was first created */
  startTime?: number;
  /** Timestamp (ms) when this content part was completed */
  endTime?: number;
  /** Input (prompt) tokens consumed by the LLM call that produced this content part.
   *  Excludes cache tokens; see cacheCreationTokens / cacheReadTokens. */
  inputTokens?: number;
  /** Output (completion) tokens produced by the LLM call that produced this content part */
  outputTokens?: number;
  /** Prompt-cache write tokens (cache_creation) from the LLM call that produced this part.
   *  Charged at the cache-write rate. 0 when the provider does not report cache data. */
  cacheCreationTokens?: number;
  /** Prompt-cache read tokens (cache_read) from the LLM call that produced this part.
   *  Charged at the discounted cache-read rate. 0 when the provider does not report cache data. */
  cacheReadTokens?: number;
};

/** Metadata for parallel content rendering and timing - subset of PartMetadata */
