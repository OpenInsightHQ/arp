...
export const useGetPIFilesQuery = (
  agentId: string,
  sessionId: string,
  config?: UseQueryOptions<dataService.PIFilesResponse>,
): QueryObserverResult<dataService.PIFilesResponse> => {
  return useQuery<dataService.PIFilesResponse>(
    [QueryKeys.piFiles, agentId, sessionId],
    () => dataService.getPIFiles(agentId, sessionId),
    {
...
export const useDeletePIFileMutation = (): UseMutationResult<
  void,
  unknown,
  { agentId: string; sessionId: string; filename: string }
> => {
  const queryClient = useQueryClient();
  return useMutation(
    ({ agentId, sessionId, filename }: { agentId: string; sessionId: string; filename: string }) =>
      dataService.deletePIFile(agentId, sessionId, filename),
    {
      onSuccess: (_data, variables) => {
        queryClient.invalidateQueries([QueryKeys.piFiles, variables.agentId, variables.sessionId]);
      },
    },
  );
};
...
export const useListFilesQuery = (
  ...
  sessionId: string,
  path?: string,
  config?: UseQueryOptions<dataService.FileManagementListResponse>,
): QueryObserverResult<dataService.FileManagementListResponse> => {
  return useQuery<dataService.FileManagementListResponse>(
    [QueryKeys.files, agentId, sessionId, path ?? ''],
    () => dataService.listFiles(agentId, sessionId, path),
    {
      enabled: !!agentId && !!sessionId && sessionId !== Constants.NEW_CONVO,
      refetchOnWindowFocus: false,
      staleTime: 30000,
      ...config,
    },
...
export const useSearchFilesQuery = (
  ...
  pattern?: string,
  type?: string,
  config?: UseQueryOptions<dataService.FileManagementSearchResponse>,
): QueryObserverResult<dataService.FileManagementSearchResponse> => {
  return useQuery<dataService.FileManagementSearchResponse>(
    [QueryKeys.files, 'search', agentId, sessionId, searchPath ?? '', pattern ?? '', type ?? ''],
    () => dataService.searchFiles(agentId, sessionId, searchPath, pattern, type),
    {
      enabled: !!agentId && !!sessionId && sessionId !== Constants.NEW_CONVO,
      refetchOnWindowFocus: false,
      ...config,
...
export const useGetFileDetailsQuery = (
  agentId: string,
  sessionId: string,
  filePath: string,
  config?: UseQueryOptions<dataService.FileManagementDetailsResponse>,
): QueryObserverResult<dataService.FileManagementDetailsResponse> => {
  return useQuery<dataService.FileManagementDetailsResponse>(
    [QueryKeys.files, 'details', agentId, sessionId, filePath],
    () => dataService.getFileDetails(agentId, sessionId, filePath),
    {
      enabled: !!agentId && !!sessionId && !!filePath,
      refetchOnWindowFocus: false,
      ...config,
    },
  );
...
export const useCreateFolderMutation = (): UseMutationResult<
  dataService.FileManagementMkdirResponse,
  unknown,
  dataService.FileManagementMkdirBody
> => {
  const queryClient = useQueryClient();
  return useMutation((body: dataService.FileManagementMkdirBody) => dataService.createFolder(body), {
    onSuccess: (_data, variables) => {
      queryClient.invalidateQueries([QueryKeys.files, variables.agentId, variables.sessionId]);
    },
  });
};

export const useRenameFileMutation = (): UseMutationResult<
  dataService.FileManagementRenameResponse,
  unknown,
  dataService.FileManagementRenameBody
> => {
  const queryClient = useQueryClient();
  return useMutation((body: dataService.FileManagementRenameBody) => dataService.renameFile(body), {
    onSuccess: (_data, variables) => {
      queryClient.invalidateQueries([QueryKeys.files, variables.agentId, variables.sessionId]);
    },
  });
...
export const useMoveFilesMutation = (): UseMutationResult<
  dataService.FileManagementMoveResponse,
  unknown,
  dataService.FileManagementMoveBody
> => {
  const queryClient = useQueryClient();
  return useMutation((body: dataService.FileManagementMoveBody) => dataService.moveFiles(body), {
    onSuccess: (_data, variables) => {
      queryClient.invalidateQueries([QueryKeys.files, variables.agentId, variables.sessionId]);
    },
  });
...
export const useDeleteFileMutation = (): UseMutationResult<
  dataService.FileManagementDeleteResponse,
  unknown,
  dataService.FileManagementDeleteBody
> => {
  const queryClient = useQueryClient();
  return useMutation((body: dataService.FileManagementDeleteBody) => dataService.deleteFile(body), {
    onSuccess: (_data, variables) => {
      queryClient.invalidateQueries([QueryKeys.files, variables.agentId, variables.sessionId]);
    },
  });
...
export const useBatchDeleteFilesMutation = (): UseMutationResult<
  dataService.FileManagementBatchDeleteResponse,
  unknown,
  dataService.FileManagementBatchDeleteBody
> => {
  const queryClient = useQueryClient();
  return useMutation(
    (body: dataService.FileManagementBatchDeleteBody) => dataService.batchDeleteFiles(body),
    {
      onSuccess: (_data, variables) => {
        queryClient.invalidateQueries([QueryKeys.files, variables.agentId, variables.sessionId]);
      },
...
export const useUnzipFileMutation = (): UseMutationResult<
  dataService.FileManagementUnzipResponse,
  unknown,
  dataService.FileManagementUnzipBody
> => {
  const queryClient = useQueryClient();
  return useMutation((body: dataService.FileManagementUnzipBody) => dataService.unzipFile(body), {
    onSuccess: (_data, variables) => {
      queryClient.invalidateQueries([QueryKeys.files, variables.agentId, variables.sessionId]);
    },
  });
