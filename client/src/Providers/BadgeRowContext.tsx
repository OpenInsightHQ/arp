...
export default function BadgeRowProvider({
   ...
   *  Skipped when a spec is active — applyModelSpecEphemeralAgent handles both state.
   *
   *  Also copies tool state from 'new' conversation when conversationId changes. */
  useEffect(() => {
    if (isSubmitting) {
      return;
    }
    ...
    if (!hasInitializedRef.current || lastContextKeyRef.current !== storageSuffix) {
      ...
      const previousKey = lastContextKeyRef.current;
      lastContextKeyRef.current = storageSuffix;

      if (previousKey === Constants.NEW_CONVO && storageSuffix !== Constants.NEW_CONVO) {
        const keysToCopy = [
        ...
        for (const keyPrefix of keysToCopy) {
          const newKey = `${keyPrefix}${storageSuffix}`;
          const oldKey = `${keyPrefix}${Constants.NEW_CONVO}`;
