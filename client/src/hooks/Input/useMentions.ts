...
export default function useMentions({
  ...
  const piMentionOption: MentionOption | null = useMemo(() => {
    if (interfaceConfig.pi !== true) {
      return null;
    }
    return {
      value: 'pi',
      label: 'ONE PI',
      type: 'pi',
      description: 'AI Agent with Skills',
      icon: EndpointIcon({
        conversation: { endpoint: 'pi' },
        endpointsConfig,
        context: 'menu-item',
        size: 20,
      }),
    };
  }, [interfaceConfig.pi, endpointsConfig]);
  ...
  const options: MentionOption[] = useMemo(() => {
    ...
    if (interfaceConfig.pi !== true) {
      validEndpoints = validEndpoints.filter((endpoint) => endpoint !== 'pi');
    }
