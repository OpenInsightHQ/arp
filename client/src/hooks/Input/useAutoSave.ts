...
export const useAutoSave = ({
  ...
  useEffect(() => {
    ...
    const isNewLike = (id: string | null) =>
      id === Constants.NEW_CONVO || id === Constants.PENDING_CONVO;

    const isPromotionFromNew =
      prevConversationIdRef.current != null &&
      isNewLike(prevConversationIdRef.current) &&
      !isNewLike(conversationId);

    if (!isPromotionFromNew) {
      setFiles(new Map());
    }
