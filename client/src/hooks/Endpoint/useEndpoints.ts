...
import { QueryKeys, TCheckUserKeyResponse } from 'librechat-data-provider';
import {
  Permissions,
  alternateName,
  EModelEndpoint,
...
export const useEndpoints = ({
  ...
  const filteredEndpoints = useMemo(() => {
    ...
    for (let i = 0; i < endpoints.length; i++) {
      ...
      if (endpoints[i] === 'pi' && interfaceConfig.pi !== true) {
        continue;
      }
    ...
    return result;
  }, [endpoints, hasAgentAccess, includedEndpoints, interfaceConfig.modelSelect, interfaceConfig.pi]);
  ...
  const isEndpointKeyAvailable = useCallback(
    (ep: string): boolean => {
      ...
      const userProvidesKey = endpointRequiresUserKey(ep);
      if (!userProvidesKey) {
        return true;
      }
    ...
    [queryClient, endpointRequiresUserKey, endpointsConfig],
  );

  const filteredEndpointsWithKey = useMemo(() => {
    const availableEndpoints = filteredEndpoints.filter((ep) => isEndpointKeyAvailable(ep));
    return availableEndpoints;
  }, [filteredEndpoints, isEndpointKeyAvailable]);

  const mappedEndpoints: Endpoint[] = useMemo(() => {
    return filteredEndpointsWithKey.map((ep) => {
