...
export default function useStepHandler({
  ...
  const updateContent = (
    ...
    if (metadata != null && (metadata.agentId != null || metadata.groupId != null)) {
    ...
    const part = updatedContent[index] as TMessageContentParts & ContentMetadata;
    if (part) {
      if (isNewPart || part.startTime == null) {
        part.startTime = now;
      }
      if (finalUpdate) {
        part.endTime = now;
  ...
  const stepHandler = useCallback(
    ({ event, data }: TStepEvent, submission: EventSubmission) => {
      ...
      if (event === 'on_run_step') {
      ...
      } else if (event === 'pi_thinking') {
        const thinkingData = data as {
          type?: string;
          contentIndex?: number;
          delta?: string;
          content?: string;
        ...
        // For thinking_end, use complete content; for thinking_delta, use delta
        const isThinkingEnd = thinkingData.type === 'thinking_end';
        const thinkContent = isThinkingEnd
        ...
        // Find or create THINK content part
        const existingContent = [...(response.content || [])] as TMessageContentParts[];
        let thinkIndex = existingContent.findIndex((part) => part.type === ContentTypes.THINK);

        if (thinkIndex === -1) {
          thinkIndex = existingContent.length;
          existingContent.push({ type: ContentTypes.THINK, think: '' } as TMessageContentParts);
        }

        const currentThinkPart = existingContent[thinkIndex] as {
          type: typeof ContentTypes.THINK;
          think: string;
        };

        if (isThinkingEnd) {
          // For thinking_end, replace with complete content
          existingContent[thinkIndex] = {
            type: ContentTypes.THINK,
            think: thinkContent,
          } as TMessageContentParts;
        } else {
          // For thinking_delta, append
          existingContent[thinkIndex] = {
            type: ContentTypes.THINK,
            think: (currentThinkPart.think ?? '') + thinkContent,
          } as TMessageContentParts;
        }
        ...
        const toolCallId = toolData.toolCallId ?? `pi_tool_${Date.now()}`;
        const toolName = toolData.toolName ?? 'pi_tool';
        const args =
          typeof toolData.args === 'object'
            ? JSON.stringify(toolData.args, null, 2)
            : (toolData.args ?? '');

        // Find or create TOOL_CALL content part
        const existingContent = [...(response.content || [])] as TMessageContentParts[];
        let toolCallIndex = existingContent.findIndex(
          (part) =>
            part.type === ContentTypes.TOOL_CALL &&
            (part as Agents.ToolCallContent).tool_call?.id === toolCallId,
        );

        if (toolCallIndex === -1) {
          toolCallIndex = existingContent.length;
        ...
        const existingToolCall = (existingContent[toolCallIndex] as Agents.ToolCallContent)
          ?.tool_call;
        const isFinal = toolData.event === 'tool_end';

        const updatedToolCall: Agents.ToolCall & PartMetadata = {
          id: toolCallId,
          name: toolName,
          args: isFinal ? args : (existingToolCall?.args ?? '') + args,
          type: ToolCallTypes.TOOL_CALL,
        };

        if (isFinal) {
          updatedToolCall.progress = 1;
          // Format result for display
          let output = '';
          if (toolData.result) {
            if (typeof toolData.result === 'object') {
              output = JSON.stringify(toolData.result, null, 2);
            } else {
              output = String(toolData.result);
            }
          }
          if (toolData.isError) {
            output = `Error: ${output}`;
          }
          updatedToolCall.output = output;
        } else {
          updatedToolCall.progress = 0.5;
        }
