...
export default function useResumeOnLoad(
  ...
  useEffect(() => {
    ...
    // Don't process the same conversation twice UNLESS the user navigated away and back
    // Reset processedConvoRef when conversationId changes (user switched conversations)
    // This allows re-resuming when switching back to a convo with an active job
    if (processedConvoRef.current === conversationId) {
      // Already processed - but check if there's still an active job
      // If the user switched away and back, the SSE connection may have been closed
      // and we need to resume again
      if (isSuccess && streamStatus?.active && streamStatus?.streamId) {
        console.log('[ResumeOnLoad] Re-resuming active job for:', conversationId, {
          streamId: streamStatus.streamId,
        });
        // Don't return - fall through to create a new submission
        processedConvoRef.current = null; // Allow re-processing
      } else {
        console.log('[ResumeOnLoad] Skipping - already processed and no active job');
        return;
      }
    }
