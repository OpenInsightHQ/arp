  ...
  const finalHandler = useCallback(
    (data: TFinalResData, submission: EventSubmission) => {
      ...
      try {
        ...
        // Ensure responseMessage has updatedAt for TIME display
        if (responseMessage && !responseMessage.updatedAt) {
          responseMessage.updatedAt = new Date().toISOString();
        }
        // Determine the base messages for final assembly.
        // Use getMessages() when the user is still on the same conversation
        // to avoid stale closure snapshots (fixes branch/duplicate issue when
        // user mutates state mid-stream via @mention, delete, etc.).
        // Fall back to the submission snapshot when the user has navigated away.
        const currentConvoId = conversation.conversationId ?? submissionConvo.conversationId;
        const activeConvoId = currentConvoId;
        const isActiveConvo = activeConvoId && currentMessages && currentMessages.length > 0;
        const baseMessages = isActiveConvo ? currentMessages : messages;

        const replaceFinalMessages = ({
          sourceMessages,
          finalRequest,
          finalResponse,
        }: {
          sourceMessages: TMessage[];
          finalRequest?: TMessage | null;
          finalResponse?: TMessage | null;
        }) => {
          const requestMessageIds = new Set(
            [submission.userMessage?.messageId, finalRequest?.messageId].filter(Boolean),
          );
          const responseMessageIds = new Set(
            [submission.initialResponse?.messageId, finalResponse?.messageId].filter(Boolean),
          );
          let requestReplaced = finalRequest == null;
          let responseReplaced = finalResponse == null;
          const nextMessages: TMessage[] = [];

          for (let index = 0; index < sourceMessages.length; index++) {
            const message = sourceMessages[index];
            if (finalRequest != null && requestMessageIds.has(message.messageId)) {
              if (!requestReplaced) {
                nextMessages.push(finalRequest);
                requestReplaced = true;
              }
              continue;
            }

            const isTailStreamingResponse =
              index >= sourceMessages.length - 2 &&
              message.isCreatedByUser !== true &&
              message.parentMessageId === finalResponse?.parentMessageId;
            const isSameResponse =
              finalResponse != null &&
              (responseMessageIds.has(message.messageId) || isTailStreamingResponse);

            if (finalResponse != null && isSameResponse) {
              if (!responseReplaced) {
                nextMessages.push(finalResponse);
          ...
          if (finalRequest != null && !requestReplaced) {
            nextMessages.push(finalRequest);
          }

          if (finalResponse != null && !responseReplaced) {
            nextMessages.push(finalResponse);
          }
        ...
        if (runMessages) {
          ...
          finalMessages = replaceFinalMessages({
