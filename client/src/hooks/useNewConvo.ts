...
const useNewConvo = (index = 0) => {
  ...
  const isEndpointKeyAvailable = useCallback(
    (ep: string): boolean => {
      if (!endpointsConfig || Object.keys(endpointsConfig).length === 0) {
        return false;
      }
      const userProvidesKey = !!getEndpointField(endpointsConfig, ep, 'userProvide');
      if (!userProvidesKey) {
        return true;
      }
      const keyExpiry = queryClient.getQueryData<TCheckUserKeyResponse>([QueryKeys.name, ep]);
      return !!(keyExpiry?.expiresAt ?? '');
    },
    [queryClient, endpointsConfig],
  );
  ...
  const switchToConversation = useRecoilCallback(
    () =>
      async (
        ...
        if (buildDefaultConversation) {
          ...
          // Filter out endpoints without valid API keys
          if (defaultEndpoint && !isEndpointKeyAvailable(defaultEndpoint)) {
            const availableEndpoint = Object.keys(endpointsConfig ?? {}).find((ep) =>
              isEndpointKeyAvailable(ep as EModelEndpoint),
            ) as EModelEndpoint | undefined;
            if (availableEndpoint) {
              defaultEndpoint = availableEndpoint;
            } else {
          ...
          if (!defaultEndpoint) {
            ...
            defaultEndpoint = Object.keys(endpointsConfig ?? {}).find((ep) => {
              ...
              return !!endpointsConfig?.[ep] && isEndpointKeyAvailable(ep as EModelEndpoint);
            }) as EModelEndpoint;
          }
    ...
    [navigate, endpointsConfig, defaultPreset, assistantsListMap, modelsQuery.data, hasAgentAccess],
  );
  ...
  const newConversation = useCallback(
    function createNewConvo({
      ...
      let defaultModelSpec = result?.default ?? result?.last;

      if (
        defaultModelSpec &&
        defaultModelSpec.preset?.endpoint &&
        !isEndpointKeyAvailable(defaultModelSpec.preset.endpoint)
      ) {
        const availableSpecs = startupConfig?.modelSpecs?.list?.filter(
          (spec) => spec.preset?.endpoint && isEndpointKeyAvailable(spec.preset.endpoint),
        );
        if (availableSpecs && availableSpecs.length > 0) {
          defaultModelSpec = availableSpecs[0];
        } else {
      ...
      // 检查 URL 参数中是否有 file_info（文件信息），如果有则保留文件（不清空）
      const fileInfoParam = searchParams.get('file_info');
      const hasFileInfoParam = !!fileInfoParam;
      ...
      if (conversation.conversationId === Constants.NEW_CONVO && !modelsData) {
        ...
        // 如果 URL 中有 file_info 参数，保留现有文件不清空
        // 这样 ArtifactDetail 设置的文件可以被保留下来
        if (!hasFileInfoParam) {
          setFiles(new Map());
          ...
          console.log('[useNewConvo] 保留文件 (file_info param detected)');
        }
      }
