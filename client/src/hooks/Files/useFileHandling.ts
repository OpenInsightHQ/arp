...
const useFileHandling = (params?: UseFileHandling) => {
  ...
  const startUpload = async (extendedFile: ExtendedFile) => {
    ...
    if (!isAssistantsEndpoint(endpointType ?? endpoint)) {
      const effectiveAgentId = agent_id || conversation?.agent_id;
      const isRealAgent = effectiveAgentId && !isEphemeralAgentId(effectiveAgentId);

      const tool_resource = extendedFile.tool_resource;
      const isMessageAttachment = tool_resource == null;

      if (isRealAgent) {
        formData.append('agent_id', effectiveAgentId);
      ...
      if (isRealAgent) {
        // Real agent (starts with "agent_"), use as-is
      } else if (conversation?.model != null) {
        const modelLabel = conversation?.modelLabel ?? '';
        const endpointsConfig = queryClient.getQueryData<TEndpointsConfig>([QueryKeys.endpoints]);
        const modelDisplayLabel = endpointsConfig?.[endpoint ?? '']?.modelDisplayLabel ?? '';
        ...
        formData.append('modelLabel', sender ?? '');
        formData.append('ephemeralAgentId', ephemeralId);
      }
