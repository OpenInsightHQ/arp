...
const resourceToPermissionMap: Partial<Record<ResourceType, PermissionTypes>> = {
  ...
  [ResourceType.GALLERY_ARTIFACT]: PermissionTypes.GALLERY_ARTIFACTS,
};
...
export const useCanSharePublic = (resourceType: ResourceType): boolean => {
  ...
  // GalleryArtifact: 直接返回 true，不检查权限
  return resourceType === ResourceType.GALLERY_ARTIFACT ? true : hasAccess;
};
