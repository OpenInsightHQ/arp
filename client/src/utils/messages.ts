...
export type GetSiblingIdxFn = (parentMessageId: string | null | undefined) => number;

/**
 * Collects user messages along the currently visible branch of the message tree,
 * matching MultiMessage sibling selection at each level.
 */
export function extractVisibleUserMessages(
  tree: TMessage[] | null | undefined,
  parentMessageId: string | null | undefined,
  getSiblingIdx: GetSiblingIdxFn,
  messages: TMessage[],
): void {
  ...
  let siblingIdx = getSiblingIdx(parentMessageId);
  if (siblingIdx >= tree.length) {
    siblingIdx = 0;
  }

  const message = tree[tree.length - siblingIdx - 1];
  if (!message) {
    return;
  }
  ...
  if (message.children && message.children.length > 0) {
    extractVisibleUserMessages(message.children, message.messageId, getSiblingIdx, messages);
  }
}
...
export const clearMessagesCache = (
  ...
  // [DEBUG-SSE] 记录缓存清除时的上下文
  const cachedMessages = queryClient.getQueryData<TMessage[]>([QueryKeys.messages, convoId]);
  const activeJobs = queryClient.getQueryData(['active-jobs']);
  console.warn('[clearMessagesCache] called', {
    conversationId: convoId,
    cachedCount: cachedMessages?.length ?? 0,
    activeJobs: activeJobs ? JSON.stringify(activeJobs) : 'null',
    stackTrace: new Error('stack').stack?.split('\n').slice(2, 5).join(' | '),
  });
