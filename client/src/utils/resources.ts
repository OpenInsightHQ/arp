...
export const RESOURCE_CONFIGS: Record<ResourceType, ResourceConfig> = {
  ...
  [ResourceType.GALLERY_ARTIFACT]: {
    resourceType: ResourceType.GALLERY_ARTIFACT,
    defaultViewerRoleId: AccessRoleIds.GALLERY_ARTIFACT_VIEWER,
    defaultEditorRoleId: AccessRoleIds.GALLERY_ARTIFACT_EDITOR,
    defaultOwnerRoleId: AccessRoleIds.GALLERY_ARTIFACT_OWNER,
    getResourceUrl: (artifactId: string) => `${window.location.origin}/artifacts/${artifactId}`,
    getResourceName: (name?: string) => (name && name !== '' ? name : '作品'),
    getShareMessage: (name?: string) => (name && name !== '' ? name : '作品'),
    getManageMessage: (name?: string) =>
      `管理 ${name && name !== '' ? name : '作品'} 的权限`,
    getCopyUrlMessage: () => '作品链接已复制',
  },
};
