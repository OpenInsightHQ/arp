...
import { PermissionTypes, Permissions, QueryKeys, SystemRoles } from 'librechat-data-provider';
...
const AgentMarketplace: React.FC<AgentMarketplaceProps> = ({ className = '' }) => {
  ...
  const isUserRole = user?.role === SystemRoles.USER;
  ...
  const slogans = [
    localize('com_arp_slogan1'),
    localize('com_arp_slogan2'),
    localize('com_arp_slogan3'),
  ];
  ...
  useEffect(() => {
    ...
    const typingSpeed = 150; // 慢一点：150ms per character
    const deletingSpeed = 50; // 删除速度快一点
    const pauseDuration = 4000; // 停留4秒
    ...
    if (isPaused) {
      ...
      if (charIndex > 0) {
        ...
        // 删除完成，切换到下一句
        setIsDeleting(false);
        setCurrentIndex((currentIndex + 1) % slogans.length);
        setCharIndex(0);
      }
    } else {
      // 输入文字
      if (charIndex < currentSlogan.length) {
        ...
        // 然后判断是否需要停顿
        const char = currentSlogan[charIndex];
  ...
  }, [charIndex, isDeleting, isPaused, currentIndex]);

  // Update typedText based on charIndex and currentIndex
  useEffect(() => {
    setTypedText(slogans[currentIndex].substring(0, charIndex));
  }, [charIndex, currentIndex, slogans]);
  ...
  // Ensure right sidebar is always visible in marketplace (only if user has agent access and not USER role)
  useEffect(() => {
    if (hasAccessToMarketplace && !isUserRole) {
      setHideSidePanel(false);
      localStorage.setItem('hideSidePanel', 'false');
      localStorage.setItem('fullPanelCollapse', 'false');
    }
  }, [setHideSidePanel, hideSidePanel, hasAccessToMarketplace, isUserRole]);
        ...
        >
          <main className="flex h-full flex-col overflow-hidden" role="main">
            ...
            >
              ...
              {!isSmallScreen && (
                <div className="sticky top-0 z-20 flex items-center justify-between bg-presentation p-2 font-semibold text-text-primary md:h-14">
              ...
              {!isSmallScreen && (
                <div className="container mx-auto max-w-4xl">
                  <div className={cn('mb-8 text-center', 'mt-12')}>
                    ...
                    >
                      {typedText}
                      <span className="ml-1 inline-block w-2 h-4 align-middle bg-text-primary animate-pulse"></span>
                    </p>
                  </div>
                </div>
              )}
              ...
              >
                <div className="container mx-auto max-w-4xl px-4">
                  ...
                  <div className="mx-auto flex max-w-2xl gap-2 pb-6">
                    <SearchBar value={searchQuery} onSearch={handleSearch} placeholder={localize('com_agents_search_placeholder')} />
