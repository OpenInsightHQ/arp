...
const SearchBar: React.FC<SearchBarProps> = ({ value, onSearch, className = '', placeholder }) => {
  ...
  // Update internal state when props change (external update like clicking recent search)
  useEffect(() => {
    if (value !== searchTerm) {
  ...
  useEffect(() => {
    // Skip if this was an external update (parent already has the correct value)
  ...
  const inputPlaceholder = placeholder || localize('com_agents_search_placeholder');
