...
interface MyPromptsEmptyStateProps {
  isFiltered?: boolean;
}

export default function MyPromptsEmptyState({ isFiltered = false }: MyPromptsEmptyStateProps) {
  ...
  return (
    <div className="flex flex-col items-center justify-center rounded-lg border border-border-light bg-transparent p-6 text-center">
      ...
      {isFiltered ? (
        <p className="text-sm text-text-secondary">{localize('com_ui_no_prompts_match')}</p>
      ) : (
        <>
