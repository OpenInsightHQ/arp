...
export default function GenericGrantAccessDialog({
  ...
  // Ensure current user is always in the shares list as owner if no owner exists
  const { data: userData } = useAuthContext();
  
  React.useEffect(() => {
    if (!isLoadingPermissions && permissionsData && userData && config?.defaultOwnerRoleId) {
      const currentUserShare = allShares.find(
        (share) => share.type === 'user' && share.id === userData.id
      );
      
      // If no owner exists and current user is not in shares, add them as owner
      if (!hasAtLeastOneOwner && !currentUserShare && !hasChanges) {
        setAllShares((prev) => [
          ...prev,
          {
            type: 'user',
            ...
            idOnTheSource: userData.id,
            accessRoleId: config.defaultOwnerRoleId,
            isExisting: false,
          },
        ]);
        setHasChanges(true);
  ...
  }, [isLoadingPermissions, permissionsData, userData, config, hasAtLeastOneOwner, hasChanges, allShares]);
        ...
        <div className="space-y-6 p-2">
          ...
          <div className="flex justify-between pt-4">
            <div className="flex gap-2">
              {!hideCopyUrl && resourceId && resourceUrl && (
                <Button
                  variant="outline"
                  onClick={() => {
                    if (isCopying) return;
            ...
            <div className="flex gap-2" style={{ position: 'relative', zIndex: 0 }}>
              {!hideAdminSettings && <PeoplePickerAdminSettings />}
