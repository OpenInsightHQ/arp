...
import EChartsErrorBoundary from '~/components/Messages/Content/EChartsErrorBoundary';
...
const stringifyCodeContent = (children: React.ReactNode): string => {
...
const isEChartsJSON = (lang: string | null, content: string): boolean => {
  if (lang === 'echarts') {
    return true;
  }
  if (lang !== 'json') {
  ...
  try {
    ...
    return Boolean(
      parsed != null &&
        ...
        (parsed as Record<string, unknown>).__echarts === true,
    );
  } catch {
    return false;
  }
...
export const code: React.ElementType = memo(({ className, children }: TCodeProps) => {
  ...
  const content = useMemo(() => stringifyCodeContent(children), [children]);
  const isECharts = useMemo(() => isEChartsJSON(lang, content), [lang, content]);
  ...
  if (isMath) {
    ...
    return (
      <EChartsErrorBoundary code={content}>
...
export const a: React.ElementType = memo(({ href, children }: TAnchorProps) => {
  ...
  // AuthContext may not exist in share pages; token is optional
  ...
  // PI file download link
  if (href?.includes('/api/pi/files/download')) {
    const piFilename = (() => {
      try {
        const url = new URL(href, window.location.origin);
        return url.searchParams.get('path') || 'download';
      } catch { return 'download'; }
    })();
    const handlePiDownload = async (event: React.MouseEvent<HTMLAnchorElement>) => {
      ...
      try {
        ...
        link.href = url;
        link.setAttribute('download', piFilename);
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
        window.URL.revokeObjectURL(url);
      } catch (error) {
        console.error('PI file download error:', error);
        showToast({ status: 'error', message: localize('com_ui_download_error') });
      }
    };
    return (
      <a href={href} onClick={handlePiDownload} target="_blank"
  ...
  const {
    ...
    const match = href?.match(pattern) ?? null;
    if (match && match[0]) {
      const path = match[0];
