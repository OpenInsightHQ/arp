...
import { EditTextPart, EmptyText, ThinkingProcess, AttachmentGroup } from './Parts';
...
const ContentParts = memo(function ContentParts({
  ...
  const { toolCallVisible } = useAgentConfig();
  ...
  if (groupedRender) {
    // Collect attachments from TOOL_CALL parts that are hidden inside ThinkingProcess
    const toolCallAttachments: TAttachment[] = [];
    groupedRender.thinkingParts.forEach((part) => {
      if (part.type === ContentTypes.TOOL_CALL) {
        const toolCallId = (part?.[ContentTypes.TOOL_CALL] as Agents.ToolCall | undefined)?.id ?? '';
        const partAttachments = attachmentMap[toolCallId];
        if (partAttachments) {
          toolCallAttachments.push(...partAttachments);
        }
      }
    });
    ...
    return (
      <SearchContext.Provider value={{ searchResults }}>
        <MemoryArtifacts attachments={attachments} conversationId={conversationId} />
        ...
        <ThinkingProcess
          parts={groupedRender.thinkingParts}
          isSubmitting={effectiveIsSubmitting}
          isLatestMessage={isLatestMessage ?? false}
        />
        {toolCallAttachments.length > 0 && <AttachmentGroup attachments={toolCallAttachments} />}
        {groupedRender.textParts.map(({ part, idx }) =>
  ...
  // Normal mode (toolCallVisible = true): render all parts sequentially
