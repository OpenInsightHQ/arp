...
export default function ToolCall({
  ...
  attachments,
  auth,
  toolCallVisible: _toolCallVisibleProp = true,
}: {
  ...
  const toolCallVisible = _toolCallVisibleProp && agentConfig.toolCallVisible;
  ...
  return (
    <>
      <div className="relative my-2.5 flex h-5 shrink-0 items-center gap-2.5">
        <ProgressText
          progress={progress}
          onClick={() => toolCallVisible && setShowInfo((prev) => !prev)}
          inProgressText={
            function_name
          ...
          hasInput={toolCallVisible && hasInfo}
          isExpanded={showInfo}
          error={cancelled}
        />
      </div>
