...
export default function MemoryArtifacts({
  ...
  const { hasErrors, memoryArtifacts, messageId, suggestions } = useMemo(() => {
    let hasErrors = false;
    const result: MemoryArtifact[] = [];
    const suggestionList: { artifact: MemoryArtifact; messageId: string }[] = [];
    let msgId = '';

    if (!attachments || attachments.length === 0) {
      return { hasErrors, memoryArtifacts: result, messageId: msgId, suggestions: suggestionList };
    }
    ...
    for (const attachment of attachments) {
      if (attachment?.[Tools.memory] != null) {
        ...
        if (artifact.type === 'suggestion') {
          suggestionList.push({ artifact, messageId: attachment.messageId || '' });
        } else {
