...
import { useDeleteFileMutation } from 'librechat-data-provider/react-query';
...
export default function FileRow({
  ...
  const { mutateAsync: deletePiFile } = useDeleteFileMutation();
          ...
          .uniqueFiles.map((file: ExtendedFile, index: number) => {
            const handleDelete = () => {
              ...
              if (file.endpoint === 'pi') {
                ...
                if (file.pi_agent_id && file.pi_session_id && file.filepath) {
                  deletePiFile({
                    agentId: file.pi_agent_id,
                    sessionId: file.pi_session_id,
                    path: file.filepath,
                  }).catch((err) => {
                    console.error('PI file delete failed:', err);
                  });
                }
                return;
              }
