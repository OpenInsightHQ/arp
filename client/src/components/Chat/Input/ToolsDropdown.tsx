...
import { Globe, LineChart, Settings, Settings2, TerminalSquareIcon } from 'lucide-react';
...
const ToolsDropdown = ({ disabled }: ToolsDropdownProps) => {
  ...
  const {
    ...
    setEnabled: setEchartsEnabled,
    isPinned: isEchartsPinned,
    setIsPinned: setIsEchartsPinned,
  } = useEchartsPrompt();
  const {
  ...
  const handleEchartsToggle = useCallback(() => {
    setEchartsEnabled(!echartsEnabled);
  }, [echartsEnabled, setEchartsEnabled]);
  ...
  dropdownItems.push({
    onClick: handleEchartsToggle,
    hideOnClick: false,
    render: (props) => (
      <div {...props}>
        <div className="flex items-center gap-2">
          <LineChart className="icon-md" aria-hidden="true" />
          <span>{localize('com_ui_echarts')}</span>
        </div>
        <button
          type="button"
          onClick={(e) => {
            e.stopPropagation();
            setIsEchartsPinned(!isEchartsPinned);
          }}
          className={cn(
            'rounded p-1 transition-all duration-200',
            'hover:bg-surface-secondary hover:shadow-sm',
            !isEchartsPinned && 'text-text-secondary hover:text-text-primary',
          )}
          aria-label={isEchartsPinned ? 'Unpin' : 'Pin'}
        >
          <div className="h-4 w-4">
            <PinIcon unpin={isEchartsPinned} />
          </div>
        </button>
      </div>
    ),
