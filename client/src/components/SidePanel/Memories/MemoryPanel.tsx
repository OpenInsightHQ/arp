...
import { SystemRoles, PermissionTypes, Permissions, QueryKeys } from 'librechat-data-provider';
import {
  Button,
  Switch,
  Spinner,
...
export default function MemoryPanel() {
  ...
  // Only fetch memories when user has read access, preventing 403 errors and infinite spinner
  const { data: memData, isLoading, isError } = useMemoriesQuery({
  ...
  // Clear stale cached memories data when access is revoked
  useEffect(() => {
    if (!hasReadAccess) {
      queryClient.removeQueries([QueryKeys.memories]);
    }
  }, [hasReadAccess, queryClient]);

  // Check read access FIRST - before any loading state
  // This prevents the infinite spinner when permissions are disabled
  if (!hasReadAccess) {
    return (
