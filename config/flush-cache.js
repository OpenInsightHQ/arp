...
const getRedisCA = () => {
  ...
  const safeRedisCA = sanitizeForLog(String(REDIS_CA));
  if (path.normalize(safeRedisCA).includes('..')) {
    console.error(`❌ Invalid REDIS_CA path (path traversal detected): ${safeRedisCA}`);
    return null;
  }
  if (!/\.(crt|pem|cer|key)$/i.test(safeRedisCA)) {
    console.error(`❌ Invalid REDIS_CA path (only .crt, .pem, .cer, .key files allowed): ${safeRedisCA}`);
    return null;
  }
  try {
    if (fs.existsSync(safeRedisCA)) {
      return fs.readFileSync(safeRedisCA, 'utf8');
    } else {
      console.warn(`⚠️  Redis CA certificate file not found: ${safeRedisCA}`);
      return null;
    }
  } catch (error) {
    console.error(`❌ Failed to read Redis CA certificate file '${safeRedisCA}':`, error.message);
    return null;
  }
};
