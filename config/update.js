...
const { askQuestion, isDockerRunning, deleteNodeModules, silentExit, runCommand } = require('./helpers');
...
(async () => {
  ...
  if (docker) {
    ...
    const composeFileArgs = singleCompose ? ['-f', './docs/dev/single-compose.yml'] : [];
    const downDisplay = `${useSudo ? 'sudo ' : ''}docker compose ${
      singleCompose ? '-f ./docs/dev/single-compose.yml ' : ''
    }down`;
    console.orange(downDisplay);
    runCommand('docker', ['compose', ...composeFileArgs, 'down'], { sudo: useSudo });
    console.purple('Pruning all LibreChat Docker images...');

    const imageName = singleCompose ? 'librechat_single' : 'librechat';
    try {
      runCommand('docker', ['rmi', `${imageName}:latest`], { sudo: useSudo });
    } catch (e) {
    ...
    runCommand('docker', ['image', 'prune', '-f'], { sudo: useSudo });
    console.purple('Building new LibreChat image...');
    const buildDisplay = `${useSudo ? 'sudo ' : ''}docker compose ${
      singleCompose ? '-f ./docs/dev/single-compose.yml ' : ''
    }build --no-cache`;
    console.orange(buildDisplay);
    runCommand('docker', ['compose', ...composeFileArgs, 'build', '--no-cache'], { sudo: useSudo });
  } else {
  ...
  if (docker) {
    startCommand = `${useSudo ? 'sudo ' : ''}docker compose ${
      singleCompose ? '-f ./docs/dev/single-compose.yml ' : ''
    }up`;
  }
