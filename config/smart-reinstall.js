...
function installDeps(hash) {
  ...
  runCommand('npm', ['cache', 'clean', '--force'], { cwd: ROOT_DIR });
...
function runTurboBuild() {
  ...
  console.gray(`      npx turbo ${args.join(' ')}\n`);
  runCommand('npx', ['turbo', ...args], { cwd: ROOT_DIR });
}

function cleanTurboCache() {
  console.purple('Clearing Turborepo cache...');
  try {
    runCommand('npx', ['turbo', 'daemon', 'stop'], { cwd: ROOT_DIR, stdio: 'pipe' });
  } catch {
  ...
  try {
    runCommand('npx', ['turbo', 'clean'], { cwd: ROOT_DIR, stdio: 'pipe' });
