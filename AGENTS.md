...
- pi endpoint flows (`endpoint === 'pi'`, detected via the `isPIEndpoint` client option from `initialize.js`, or the agent's endpoint) do **not** write these fields — the pi backend persists the authoritative per-call/turn values on the same documents, and arp's view through the OpenAI translation layer loses per-call and cache detail. For live display, `ResumableAgentController` (`api/server/controllers/agents/request.js`, `mergePiUsageFields`) reads the pi-persisted message back after the stream ends and merges the usage fields into the FINAL event (display-only; no DB write) — pi finalizes its document writes before its SSE stream ends, so the values are already final. The chat page tooltip (`client/src/components/Chat/Messages/TokenUsageTooltip.tsx`) renders turn totals (`total*`) only, with the same values driving the In/Out summary.
...
- Provider `input_tokens` semantics differ by family and are handled by `inputTokensExcludeCache` (`packages/api/src/utils/content.ts`): Claude-style entries EXCLUDE cache from `input_tokens` (cache is added to obtain the full prompt), while OpenAI-compatible entries (incl. implicit-caching models like DeepSeek/Qwen) already include cached tokens in `prompt_tokens` (cache must NOT be added — it would double count; fresh input = `input − cacheRead − cacheCreation`). This normalization applies to message fields, conversation totals, and `spendTokens`/`spendStructuredTokens` billing.
